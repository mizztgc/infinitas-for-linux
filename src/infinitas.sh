...
EOM
    ...
    infinitas_install() {
		log 0 "Downloading beatmania IIDX INFINITAS installer..."
		$WGET_EXE "$INFINITAS_LINK"
		if [[ $? -ne 0 ]]; then
			log 2 "Failed to download beatmania IIDX INFINITAS installer!"
			cleanup
			exit 2
		fi

		# extract files from installer and move them to prefix
		log 0 "Extracting files from beatmania IIDX INFINITAS installer..."
	...
	vcr2010_install() {
		log 0 "Downloading Microsoft Visual C++ Redist 2010..."
		$WGET_EXE "$VCS2010_LINK"
		if [[ $? -ne 0 ]]; then
			log 2 "Failed to download vcr2010.exe!"
			cleanup
			exit 2
		fi

		log 0 "Installing Microsoft Visual C++ Redist 2010..."
		$WINE_EXE $TEMP_DIRECTORY/vcredist*.exe '/quiet'
		if [[ $? -ne 0 ]]; then
			log 2 "Failed to install vcr2010!"
			cleanup
			exit 2
		fi
	}
	...
	dxvk_install() {
		log 0 "Downloading DXVK..."
		$WGET_EXE "$DXVK_LINK"
		if [[ $? -ne 0 ]]; then
			log 2 "Failed to download DXVK!"
			cleanup
			exit 2
		fi

		log 0 "Extracting DXVK archive..."
		$TAR_EXE -zxf "$TEMP_DIRECTORY/dxvk-2.4.tar.gz"

		log 0 "Moving .dll files..."
	...
	set_default_settings() {
		info 0 "Applying default game settings..."
		touch "$INSTALL_LOCATION/drive_c/Games/beatmania IIDX INFINITAS/Resource/config/1000_cf.xml"
...
EOF
	...
	log 0 "\033[1;92mbeatmania IIDX INFINITAS has been successfully installed!\033[0m"
	exit
}
...
launch_cmd() {
	...
	validate_wine() {
		...
		if [[ ${v:5:1} -lt 9 ]]; then
			log 2 "This script only supports Wine versions 9.0 and later"
...
start_infinitas() {
	...
	"$GS_CMD" "$WINE_EXECUTABLE" explorer "/destop=InfinityBeat,1920x1080" "$INFINITAS_LAUNCHER" "$LOGIN_TOKEN" &
